...
try {
    ...
    $whereSql = " WHERE h.maBacSi = ? AND h.trangThai = 'Đã hoàn thành' AND h.isDeleted = 0 ";
    $whereTypes = 's';
    $whereParams = [$maBacSi];
    ...
    $orderBy = "sortDate DESC, page.maHoSo DESC";
    if ($sort === 'oldest') {
        $orderBy = "sortDate ASC, page.maHoSo ASC";
    } elseif ($sort === 'name') {
        $orderBy = "sortName ASC, sortDate DESC, page.maHoSo DESC";
    }
    ...
    if (!empty($recordIds)) {
        ...
        $dataSql = "
            SELECT h.maHoSo, h.ngayHoanThanh, h.chanDoan, h.dieuTri,
